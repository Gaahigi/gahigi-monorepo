...
import { Public } from 'src/shared/decorators';
  ...
  export class AuthController {
    ...
    @Public()
    async handleRequestResetToken(@Body() { username }: RequestOTP) {
    ...
    @Public()
    async resendVerificationEmail(@Body() { username }: RequestOTP) {
      return this.authService.generateEmailOTP({
        username,
    ...
    @Patch('/verification')
    @Public()
    async verifyUser(@Body() { username, otp }: VerifyProfileDTO) {
    ...
    @Public()
    async resetPassword(@Body() { otp, username, password }: ResetPassordDTO) {
