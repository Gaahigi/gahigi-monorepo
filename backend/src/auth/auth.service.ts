...
import { generateOTP } from '@/shared/functions/otp';
  ...
  export class AuthService {
    ...
    async resetPassword({
      ...
      const formattedUsernameOrEmail = username?.trim().toLocaleLowerCase();
      const user = await this.prismaService.user.findFirst({
        where: {
          OR: [
            { username: formattedUsernameOrEmail },
     ...
     * @returns
     */
    async verifyUser({ username, otp }: { username: string; otp: string }) {
      const user = await this.prismaService.user.findFirst({
        where: {
          OR: [{ username }, { email: username }],
        },
